...
import { updateClass, type ClassResponseDto, type ClassUpdateDto } from "@/lib/admin-classes";

type EditClassModalProps = {
  isOpen: boolean;
  classData: ClassResponseDto | null;
  onClose: () => void;
  onSuccess: (updatedClass: ClassResponseDto) => void;
};

export function EditClassModal({ isOpen, classData, onClose, onSuccess }: EditClassModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await updateClass(formData);
      onSuccess(updated);
      onClose();
    } catch (err) {
      ...
      <div className="w-full max-w-lg overflow-hidden rounded-3xl border border-white/80 bg-white p-6 shadow-2xl sm:p-8">
        <div className="flex items-center justify-between pb-4 border-b border-black/5">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-foreground">Edit Class & Section</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
