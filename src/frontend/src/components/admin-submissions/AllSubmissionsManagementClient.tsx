            ...
            <div className="max-w-2xl space-y-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-7 text-(--color-muted) sm:text-base">
                  Inspect student submissions across all classes, view uploaded files, and check grades & teacher feedback.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  <select
                    value={filter.sortBy || "submittedat"}
                    onChange={(e) => {
                      const newFilter = { ...filter, sortBy: e.target.value, pageNumber: 1 };
                      setFilter(newFilter);
                      fetchSubmissionsData(newFilter);
                    }}
                    className="w-full rounded-2xl border border-slate-200 bg-white/80 px-3 py-2 text-sm font-medium text-foreground focus:border-teal-500 focus:outline-none transition shadow-2xs"
                  >
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const newFilter: SubmissionFilterDto = { ...filter, sortOrder: newOrder, pageNumber: 1 };
                      setFilter(newFilter);
                      fetchSubmissionsData(newFilter);
                    }}
                      ...
                      >
                        <td className="border-b border-black/5 px-4 py-4">
                          <div className="flex items-start gap-3">
                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-teal-100 text-xs font-bold text-teal-700 mt-0.5">
                            ...
                            <div className="flex flex-col">
                              ...
                              {sub.studentRollNo ? (
                                <span className="inline-flex w-fit items-center rounded-md border border-purple-500/20 bg-purple-500/10 px-2 py-0.5 text-[11px] font-mono font-bold text-purple-700 mt-1">
                                  Roll: {sub.studentRollNo}
                                </span>
                              ) : null}
                              <p className="text-xs text-slate-500 mt-1">{sub.studentEmail}</p>
                            </div>
                          </div>
                        </td>
                        <td className="border-b border-black/5 px-4 py-4">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs font-medium text-slate-600">{sub.className}</span>
                        ...
                        <td className="border-b border-black/5 px-4 py-4">
                          <div className="flex flex-col gap-1.5 items-start">
                            <span
                              className={`inline-flex rounded-full border px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-[0.14em] ${statusBadge(
                                sub.status
                              )}`}
                            >
                              {sub.status}
                            ...
                            {sub.grade !== undefined && sub.grade !== null ? (
                              <div className="inline-flex items-center gap-1 rounded-xl border border-emerald-200 bg-emerald-50/80 px-2.5 py-1 text-xs shadow-2xs">
                                ...
                                <span className="font-mono font-bold text-emerald-900 text-xs">{sub.grade}</span>
                                <span className="text-emerald-700 font-medium">/ {sub.maxMarks}</span>
                              </div>
                            ) : (
                              <div className="inline-flex items-center gap-1 rounded-xl border border-amber-200/80 bg-amber-50/70 px-2.5 py-1 text-xs font-medium text-amber-800 shadow-2xs">
                                <span>⏳ Ungraded ({sub.maxMarks} max)</span>
                              </div>
                            )}
                          </div>
                        </td>
                          ...
                          <div className="inline-flex items-center justify-end gap-2">
                            {sub.fileUrl && (
                              <a
                                ...
                                className="inline-flex items-center gap-1 whitespace-nowrap rounded-full border border-teal-300 bg-teal-50 px-3 py-1 text-xs font-semibold text-teal-700 shadow-2xs hover:bg-teal-600 hover:text-white transition cursor-pointer"
                              >
                                🔗 Open File
                              </a>
                            )}
