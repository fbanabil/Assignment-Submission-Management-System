...
import { createSubject, type SubjectCreateDto, type SubjectResponseDto } from "@/lib/admin-subjects";

type CreateSubjectModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (createdSubject: SubjectResponseDto) => void;
};
      ...
      <div className="w-full max-w-lg overflow-hidden rounded-3xl border border-white/80 bg-white p-6 shadow-2xl sm:p-8">
        <div className="flex items-center justify-between pb-4 border-b border-black/5">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-foreground">Create New Subject</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
