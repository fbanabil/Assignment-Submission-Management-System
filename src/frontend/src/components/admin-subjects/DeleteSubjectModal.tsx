...
import { deleteSubject, type SubjectResponseDto } from "@/lib/admin-subjects";
...
export function DeleteSubjectModal({ isOpen, subjectData, onClose, onSuccess }: DeleteSubjectModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSubject(subjectData.id);
      onSuccess(subjectData.id);
      onClose();
    } catch (err) {
      ...
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-rose-200 bg-white p-6 shadow-2xl sm:p-8">
        <div className="flex items-center justify-between pb-4 border-b border-black/5">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-foreground">Delete Subject</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="mt-5 space-y-3">
          <p className="text-sm leading-6 text-slate-600">
            Are you sure you want to delete <strong className="text-foreground">{subjectData.name}</strong> ({subjectData.code})?
          </p>
          <p className="text-xs leading-5 text-rose-600 font-medium bg-rose-50 p-3 rounded-2xl border border-rose-100">
            Warning: Removing this subject will detach it from any assigned classes and assignments.
          </p>
        </div>
